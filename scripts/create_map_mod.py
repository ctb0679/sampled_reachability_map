...
import h5py
import rospy
from scipy.spatial import KDTree  # ADDED
...
POSES_PER_VOXEL = 8  # number of orientation samples per voxel (between 30 and 50 as specified)
NUM_FK_SAMPLES = 5000  # can be adjusted for more coverage (trade-off with computation time)
...
# MODIFIED: Added init_state parameter
def inverse_kinematics(position, orientation, init_state=None):
    tf_mat = tft.quaternion_matrix(orientation)
    tf_mat[0:3,3] = position
    if init_state is None:
        init_state = [np.random.uniform(low, high) for (low, high) in JOINT_LIMITS]
    try:
...
def is_pose_reachable(query_pos, query_ori, fk_tree, fk_orientations, joint_samples, 
    ...
    # Use nearest FK sample for IK initialization
    _, closest_idx = fk_tree.query(query_pos)
    solution = inverse_kinematics(query_pos, query_ori, joint_samples[closest_idx])
...
def create_reachability_map():
    ...
    # 5. Main processing loop WITH PROGRESS LOGGING
    ...
    for i, center in enumerate(voxel_centers):
        ...
        for j, quat in enumerate(sphere_orientations):
            if is_pose_reachable(center, quat, fk_tree, fk_orientations, joint_samples):
